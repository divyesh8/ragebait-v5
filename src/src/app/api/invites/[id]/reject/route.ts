...
// POST /api/invites/:id/reject — recipient only
export async function POST(
  ...
  try {
    const rows = await sql`SELECT to_user_id, status FROM battle_invites WHERE id = ${params.id} LIMIT 1`;
    if (rows.length === 0) {
      return NextResponse.json({ error: "Invite not found." }, { status: 404 });
    }
    if (rows[0].to_user_id !== session.userId) {
    ...
    const updated = await sql`
      UPDATE battle_invites SET status = 'rejected', responded_at = now()
